...
import { addSensorData, createNewCollection , 
  ...
  addSensorCoordinates,removeSensorFromBoot,updateLabel} from './LocalMongoHandler';
...
export async function handleInfluxPull(recieved: any, ws: any): Promise<void> {
  ...
  // use queryFromBlackBox to get the data from influx
...
export function sendLaaToClient(ipv6:string,ws:any): void {
  const event = {
    type : "sp_ipv6",
    data : {
      ipv6 : ipv6,
